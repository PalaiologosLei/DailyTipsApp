...
def _run(repo_dir: Path, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    summary = run_app(
        repo_dir=repo_dir,
        notes_dir=_notes_dir_from_settings(settings),
        github_url=_github_url_from_settings(settings),
...
def _notes_dir_from_settings(settings: dict[str, Any]) -> Path | None:
    source_mode = str(settings.get("source_mode", "local"))
    if source_mode != "local":
        return None
    return _required_path(settings.get("local_path"), "Local notes directory is required in local mode.")


def _github_url_from_settings(settings: dict[str, Any]) -> str | None:
    source_mode = str(settings.get("source_mode", "local"))
    if source_mode != "github":
        return None
    github_url = str(settings.get("github_url", "")).strip()
    if not github_url:
        raise ValueError("GitHub URL is required in GitHub mode.")
    return github_url
