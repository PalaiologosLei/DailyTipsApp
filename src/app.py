...
from .cloud_sync import CloudSyncError, clear_generated_outputs, ensure_cloud_dir, update_cloud_image_index
...
def run_app(
    ...
    if notes_dir is None:
        raise AppError('Local notes directory is required.')
    if not notes_dir.exists() or not notes_dir.is_dir():
        raise AppError(f'Local notes directory does not exist: {notes_dir}')
    if cloud_dir is None:
        raise AppError('Cloud directory is required because images are published directly to the synced folder.')
    ...
    return RunSummary(
        source_description=f'local notes: {notes_dir}',
